...
if [ ! -e /var/www/html/wordpress ]; then
	...
	wp-cli core install --url=$DOMAIN_NAME --title=Inception --admin_user=$WP_ADMIN_LOGIN --admin_password=$WP_ADMIN_PASS --admin_email="$WP_ADMIN_EMAIL" --allow-root --path=/var/www/html/wordpress 
